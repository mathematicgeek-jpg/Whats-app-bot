...
async function getClient() {
  ...
  // If on Vercel and Upstash Redis is not configured, throw a clear error
  if (process.env.VERCEL) {
    throw new Error('Redis is not configured on Vercel. Please set UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN in Vercel settings.');
  }
  ...
  try {
    ...
    _client = new Redis(process.env.REDIS_URL || 'redis://localhost:6379', {
      maxRetriesPerRequest: 1 // fail fast in local dev if Redis is down
    });
